...
def summarize():
    ...
    if isinstance(data, list):  # Multiple posts
        for post in data:
         ...
         text = " ".join([data["post_title"], data["post_body"]] + data["comments"])
         summary = generate_summary(model, tokenizer, text)
         summaries.append({"title": data["post_title"], "summary": summary})

    print(f"Summaries being sent to frontend: {summaries}") # <--- ADD THIS LINE
